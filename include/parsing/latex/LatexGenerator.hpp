    ...
    class LatexGenerator : public Generator
    {
        ...
        private:
            ...
            const std::unordered_map<NodeType, String> math_operators =
            {
                ...
                {NodeType::Derivative,          STR("{\\frac{\\mathrm{d}^{#1}}{\\mathrm{d}{#0}^{#1}}}{#2}")},
                {NodeType::Integral,            STR("\\displaystyle\\int_{#0}^{#1}{#2}\\mathrm{d}{#3}")},
            ...
            const std::unordered_map<String, String> substitution_list =
            {
                ...
                {STR("Ζ"), STR("{Z}")},
                {STR("η"), STR("{\\eta}")},
                //{STR("H"), STR("{H}")},   // Identical characters left and right cause infinite loop
                {STR("θ"), STR("{\\theta}")},
                {STR("Θ"), STR("{\\Theta}")},
                {STR("ϑ"), STR("{\\vartheta}")},
                {STR("ι"), STR("{\\iota}")},
