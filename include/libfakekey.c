		     ...
		     int      flags)
{
  ...
  if (code != 0) 
    {
      utf8_fakekey_send_keyevent(fk, code, True, flags);
	      ...
	      int                  flags)
{
  ...
  if (len_bytes < 0)
    {
      ...
      */
      len_bytes = strlen((char*)utf8_char_in); 	/* OK ? */
    }
  ...
  return utf8_fakekey_press_keysym(fk, (KeySym)ucs4_out, flags);
}
...
utf8_fakekey_repeat(FakeKey *fk)
{
  ...
  utf8_fakekey_send_keyevent(fk, fk->held_keycode, True, fk->held_state_flags);
}
...
utf8_fakekey_release(FakeKey *fk)
{
  ...
  utf8_fakekey_send_keyevent(fk, fk->held_keycode, False, fk->held_state_flags);
